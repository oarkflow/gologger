...
type CustomLogger struct {
	Path string
	// Extension is the file extension; .txt, .json, .log
	Extension string
	...
	// ValueSeparator is inserted after each value in the data array of a write
	...
	// ConventionUpdated can be used to back up the old log file after the convention has change
...
func NewCustomLogger(path string, extension string, bufferSize int) *CustomLogger {
	logger := &CustomLogger{
		Extension: extension,
...
func (l *CustomLogger) convertInput(input []interface{}) []byte {
	...
	for i, x := range input {
		...
		if i != 0 {
			payload = append(payload, []byte(l.ValueSeparator)...)
		}
...
func (l *CustomLogger) getFileName() string {
	return l.Path + l.NamingConvention() + l.Extension
}
...
// Close will shut down the service worker
